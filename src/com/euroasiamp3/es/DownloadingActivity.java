...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Total Unduhan: "+totaldownloading);
        }else{
        	text.setText("Total Unduhan: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Atur Ulang Unduhan Yang Selesai & Gagal");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Unduhan Tertunda");
		    		}else if(a == 0){
		    			progress.setText("Selesai Diunduh");
		    		}else if(a == -1){
		        		progress.setText("Gagal Diunduh");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("Opsi Pengunduhan")
				       .setCancelable(false)
				       .setNegativeButton("Hapus", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("Unduhan MP3 Yang Dihapus");
				           }
				       })
				       .setNeutralButton("Coba Lagi", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Atur Untuk Coba Lagi");
				           }
				       })
				...
				final CharSequence[] items = {"Buka MP3", "Atur Sebagai Nada Dering", "Unduh Ulang", "Hapus", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Pilih Satu Opsi");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Buka MP3"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Galat Memuat MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Atur Sebagai Nada Dering"){
							...
							builder.setMessage("Yakin ingin menghapus: " + title + " ?")
							       ...
							       .setPositiveButton("Ya", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 Dihapus");
							           }
							       })
							       .setNegativeButton("Tidak", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3 Tidak Dihapus");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Unduh Ulang"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Atur Untuk Coba lagi");
				    	}
				...
				builder.setMessage("Opsi Pengunduhan")
				       .setCancelable(false)
				       .setNegativeButton("Hentikan Pengunduhan", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Sedang Diunduh", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Unduhan Tertunda", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Selesai Diunduh", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Gagal Diunduh", 100));
