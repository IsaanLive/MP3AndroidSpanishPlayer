...
public class SearchActivity extends ListActivity {
    ...
    public void startsearch(){
		...
		toastmake("Pencarian: "+searchstring);
	...
	private class RetrieveSearch extends AsyncTask<Void, Void, Void>{
		...
		protected void onCancelled() {		
			...
			TextView failresults = (TextView)findViewById(R.id.failresults);
			failresults.setText("Pencarian sudah dibatalkan");
		...
		protected void onPreExecute() {
			...
			cancelDialog.setMessage("Mencari Lagu...");
		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("Rupanya koneksi Internet Anda tidak aktif atau Lelet Sinyal Anda. Silakan aktifkan wifi atau data Anda lalu coba lagi.");
		        }else{
					failresults.setText("Hasil Tidak Ditemukan");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("Rupanya terjadi galat ketika mencoba mencari. Silakan Coba Lagi");
		        }
			}else{
				setListAdapter(adapter);
			}
		...
		public void getDilandau(String search) throws IOException, MalformedURLException, NullPointerException{
			...
			if(!sb.toString().contains("Lagu tidak ditemukan")){
